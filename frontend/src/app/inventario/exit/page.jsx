...
import { get_bodega_by_id, get_bodegas } from "@/hooks/Services_bodega";
import { get_sucursal_by_bodega_id } from "@/hooks/Services_sucursal";
import { registra_salida_inventario } from "@/hooks/Services_inventory";
...
function Inventario() {
    ...
    useEffect(() => {
        if (!isLoaded && token) {
            ...
            get_bodegas(token).then(response => {
    ...
    const handleSubmitSalida = async (e) => {
        ...
        try {
            const response = await registra_salida_inventario({
