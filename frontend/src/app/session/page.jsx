...
export default function Session() {
  ...
  let { errors } = formState;
   ...
   const sendInfo = (data) => {
    console.log("Datos enviados al backend:", data); // Ver los datos enviados
    login(data).then((info) => {
      if (info.code == '200') {
        Cookies.set('token', info.datos.token, { expires: 1 });
        Cookies.set('username', info.datos.user, { expires: 1 });
          ...
          <main className="form-signin text-center mt-5">
            <form onSubmit={handleSubmit(sendInfo)}>
              ...
              <div className="form-floating">
                <input type="text" name='username' {...register('username')} className="form-control" id="floatingInput" placeholder="name@example.com" />
                <label htmlFor="floatingInput">Usuario</label>
                {errors.username && <div className="text-xs inline-block py-1 px-2 rounded text-red-600">{errors.username?.message}</div>}
              </div>
              <div className="form-floating">
